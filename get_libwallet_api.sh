#!/bin/bash
MONERO_URL=https://github.com/swapcoinz/swapcoinz-core.git
...
MONERO_DIR=$ROOT_DIR/swapcoinz
BUILD_LIBWALLET=false

# init and update swapcoinz submodule
if [ ! -d $MONERO_DIR/src ]; then
    git submodule init swapcoinz
fi
git submodule update
git -C $MONERO_DIR fetch --tags
git -C $MONERO_DIR checkout v0.13.0

# get swapcoinz core tag
get_tag
# create local swapcoinz branch
git -C $MONERO_DIR checkout -B $VERSIONTAG

# Merge swapcoinz PR dependencies
...
git -C $MONERO_DIR config user.name "Swapcoinz GUI"
git -C $MONERO_DIR config user.email "gui@swapcoinz.local"
# check for PR requirements in most recent commit message (i.e requires #xxxx)
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    echo "Merging swapcoinz push request #$PR"
    # fetch pull request and merge
    git -C $MONERO_DIR fetch origin pull/$PR/head:PR-$PR
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge swapcoinz PR #$PR"
...
# Build libwallet if no previous version file exists
    ...
    echo "swapcoinz/version.h not found - Building libwallet"
    BUILD_LIBWALLET=true
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove swapcoinz/lib/libwallet_merged.a to force rebuild"
    fi
fi
...
echo "cleaning up existing swapcoinz build dir, libs and includes"
