...
MONERO_DIR=swapcoinz
MONEROD_EXEC=swapcoinzd
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/swapcoinz-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    MONEROD_EXEC=swapcoinzd.exe
fi
...
cd build
qmake ../swapcoinz-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy swapcoinzd to bin folder
